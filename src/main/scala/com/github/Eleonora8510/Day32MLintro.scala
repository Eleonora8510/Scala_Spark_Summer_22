...
object Day32MLintro extends App {
  ...
  val lrModel = new LogisticRegression()//here you can input some hyperparameters to adjust
    .setLabelCol("label")//this can be omitted/ skipped if your dataframe is using default label and features
    .setFeaturesCol("features")

  println(lrModel.explainParams())

  val fittedDF = lrModel.fit(train)

  fittedDF.transform(train).select("label", "prediction").show()
