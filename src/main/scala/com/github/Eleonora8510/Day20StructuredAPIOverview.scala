...
object Day20StructuredAPIOverview extends App {
  ...
  //so Collect moved the data into our own program memory because it is now an Array
...
println("First 3 elements")
  ...
  println("3rd to 7th element starting from 0")
  arrRow.slice(2,7).foreach(println) //3dr to 7th element
  println("First")
  println(arrRow.head)
  println("Last")
  ...
  val df100to3100 = range1To31.select(range1To31.col("JulyNumbers") * 100)
