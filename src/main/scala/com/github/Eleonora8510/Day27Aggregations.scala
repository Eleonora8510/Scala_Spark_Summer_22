...
import org.apache.spark.sql.functions.{approx_count_distinct, avg, col, count, countDistinct, expr, first, last, max, min, sum, sum_distinct, round}
...
object Day27Aggregations extends App {
  ...
  println("df.count == 541909", df.count() == 541909)
  ...
  df.select(min("Quantity"), max("Quantity")).show()
    ...
    df.select(sum("Quantity"), sum_distinct(col("Quantity"))).show() // 5176450
