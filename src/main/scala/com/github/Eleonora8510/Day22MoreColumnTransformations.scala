...
object Day22MoreColumnTransformations extends App {
  ...
  //val unionFirstTwo = dataFrames(0).union(dataFrames(1)) // union of first and second from our split

  val unionFirstTwo = dataFrames.head.union(dataFrames(1))
  println(s"Size of union of DataFrames is ${unionFirstTwo.count()}")
  ...
  //df.orderBy(expr("count desc")) //not working // FIXME
  df.orderBy(desc("count"), asc("DEST_COUNTRY_NAME")).show(5)//this one I would skip
