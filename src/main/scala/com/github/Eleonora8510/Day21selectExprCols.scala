...
println("Chapter 5: Select expressions. Column operations")
  ...
  val statDf = df.describe() // shows basic statistics on string and numeric column
  statDf.show()
  statDf.describe().show() // when you do not need to save the statistics dataframe for further use

  df.selectExpr("DEST_COUNTRY_NAME as newColumnName", "DEST_COUNTRY_NAME", "ORIGIN_COUNTRY_NAME as OriginCountry").show(2)
  ...
  df.selectExpr(
    "*",
  "(DEST_COUNTRY_NAME = ORIGIN_COUNTRY_NAME) as withinCountry")  // include all original columns //more useful when you only need some columns
  .show(5)
  ...
  df.selectExpr("* ").where("DEST_COUNTRY_NAME = ORIGIN_COUNTRY_NAME").show()
