...
//  connection.close()
  ...
  val dbDataFrame = spark.read
    .format("jdbc")
    .option("url", url)
    .option("dbtable", tablename)//we  are loading a single table inyo a dataframe
...
AS flight_info"""
  val dbDataFrameFromQuery = spark.read
    .format("jdbc")
    .option("url", url)
  ...
  dfFromText.show(5, false)
  dfFromText.printSchema()

  //again reading .csv should be done with csv format
