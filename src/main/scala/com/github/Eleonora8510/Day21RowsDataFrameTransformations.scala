...
object Day21RowsDataFrameTransformations extends App {
  ...
  val myManualSchema = new StructType(Array(
    ...
    StructField("names", LongType, false))) //so names have to be present it is not nullable - is required
  ...
  df.select(
    ...
    //    'DEST_COUNTRY_NAME, //not used as much required implicits which are being deprecated
    //    $"DEST_COUNTRY_NAME",
