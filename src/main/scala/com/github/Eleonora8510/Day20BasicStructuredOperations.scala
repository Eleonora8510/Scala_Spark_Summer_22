...
object Day20BasicStructuredOperations extends App {
  ...
  println(df.schema)
  df.printSchema()
  //prints the above schema a bit nicer
  ...
  //“single value” can actually be a complex type like a Map or Array.
    ...
    println("Columns in our DatFrame are: ")
  ...
  // before writing we need to go back to one partition (or we could have gone
  df.coalesce(1)
    .write
    .option("header", "true")
    .mode("overwrite")
    .csv("src/resources/flight-data/json/flight_summary_2015.csv")
